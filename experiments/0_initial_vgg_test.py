...
train_dataset  = CellPatchesDataset('~/gemelli/dataset_info/train_samples.csv',input_size=input_size,use_mask=args.use_mask)
val_dataset = CellPatchesDataset('~/gemelli/dataset_info/val_samples.csv',input_size=input_size,use_mask=args.use_mask)

assert train_dataset.n_classes==val_dataset.n_classes

num_channels = train_dataset[0][0].shape[0]
print(f'input shape = {train_dataset[0][0].shape}')
model = VggModule(input_channels=num_channels, output_classes=train_dataset.n_classes, input_size=input_size)
...
trainer.fit(
    ...
    BalancedDataLoader(val_dataset, batch_size=16,num_workers=10,shuffle=False)
)